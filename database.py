...
async def create_tables():
    async with aiosqlite.connect('Main.db') as db:
        ...
        cursor = await db.execute("CREATE TABLE IF NOT EXISTS games (game_id INTEGER AUTO_INCREMENT PRIMARY KEY, host_id TEXT, rank TEXT, status TEXT)")
        await cursor.close()
        await db.commit()

async def create_user(discord_id, elo, rank):
    async with aiosqlite.connect('Main.db') as db:
        cursor = await db.execute("INSERT INTO users VALUES (?, ?, ?, ?, ?, ?)", (discord_id, elo, rank, 0, 0, "inactive", 0))
        await cursor.close()
        await db.commit()
...
async def remove_from_queue(discord_id, game_id):
    async with aiosqlite.connect('Main.db') as db:
        cursor = await db.execute("UPDATE users SET game_id = ?, status = ? WHERE discord_id = ?", (game_id, "inactive", discord_id))
        await cursor.close()
        await db.commit()
...
async def create_game(rank):
    async with aiosqlite.connect('Main.db') as db:
        cursor = await db.execute("INSERT INTO games VALUES (?, ?, ?)", (0, rank, "in_queue"))
        await cursor.close()
        await db.commit()
